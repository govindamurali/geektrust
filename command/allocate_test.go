...
func TestAllocation_Execute(t *testing.T) {
	...
	porfolioMock.On("Allocate", portfolio.ClasswiseAllocationMap{1: 1, 2: 2, 3: 3}).Return(nil)
	allocate := allocate{amount: []int{1, 2, 3}}
	err := allocate.Execute(porfolioMock)

	assert.Nil(t, err)
	porfolioMock.AssertCalled(t, "Allocate", portfolio.ClasswiseAllocationMap{1: 1, 2: 2, 3: 3})
}
